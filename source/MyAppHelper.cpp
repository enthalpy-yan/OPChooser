...
void MyAppHelper::StrategySet(vector<Option> v,multimap<double,vector<Option> > &resultmap) {
  double LVal;
  double RVal;
  Context* pcon;
  Strategy* pStr;
...
void MyAppHelper::Order(vector<Option> v,multimap<double,vector<Option> > &resultmap) {
  int lenght= v.size();
